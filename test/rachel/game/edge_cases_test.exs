defmodule Rachel.Game.EdgeCasesTest do
  ...
  describe "deck exhaustion scenarios" do
    test "reshuffling when deck runs out during normal draw" do
      ...
      game = %GameState{
        ...
        deck: [Card.new(:hearts, 5)],  # Only 1 card left
        discard_pile: [
          Card.new(:spades, 10),
          Card.new(:hearts, 7),
          Card.new(:diamonds, 8),
    ...
    test "handling massive attack with insufficient cards" do
      ...
      game = %GameState{
        ...
        deck: Enum.take(Deck.new(), 10),  # Only 10 cards in deck
        discard_pile: Enum.take(Enum.drop(Deck.new(), 10), 20),  # 20 in discard
      ...
      assert length(player.hand) == 29  # 10 from deck + 19 from discard (keeping top card)
  ...
  describe "multiple Aces with different nominations" do
    test "second Ace overrides first nomination" do
      game = %GameState{
        ...
        nominated_suit: :clubs,  # Previous nomination
        current_player_index: 0,
        status: :playing
      }
      ...
      assert new_game.nominated_suit == nil  # Cleared after turn
    end
  ...
  describe "maximum hand size scenarios" do
    test "drawing maximum cards from stacked attacks" do
      ...
      deck = Deck.new() ++ Deck.new()  # 104 cards
  ...
  describe "complex stacking scenarios" do
    test "mixed Red and Black Jacks interaction" do
      ...
      black_attack = {:black_jacks, 15}  # 3 Black Jacks
      reduced = Rules.reduce_attack(black_attack, 2)  # 2 Red Jacks
      
      assert reduced == {:black_jacks, 5}  # 15 - 10 = 5
    end

    test "Red Jacks completely negating Black Jacks" do
      black_attack = {:black_jacks, 10}  # 2 Black Jacks
      reduced = Rules.reduce_attack(black_attack, 2)  # 2 Red Jacks
      
      assert reduced == nil  # Completely negated
    end
  ...
  describe "game ending scenarios" do
    ...
    test "last card played with special effect" do
      ...
      assert "p1" in new_game.winners
      assert new_game.pending_skips == 0  # Skip clears after turn
    end
  end
