defmodule Rachel.Game.ComplexScenariosTest do
  ...
  describe "Complex multi-turn scenarios" do
    setup do
      ...
      players = [
        %{id: "p1", name: "Player 1", hand: [
          Card.new(:hearts, 12),   # Queen (reverse)
          Card.new(:spades, 7),    # 7 (skip)
          Card.new(:clubs, 11),    # Black Jack (attack)
          Card.new(:hearts, 14),   # Ace (nominate)
          Card.new(:diamonds, 2),  # 2 (attack)
        ], status: :playing},
        %{id: "p2", name: "Player 2", hand: [
          Card.new(:diamonds, 12), # Queen (reverse)
          Card.new(:hearts, 7),    # 7 (skip counter)
          Card.new(:hearts, 11),   # Red Jack (cancel)
          Card.new(:spades, 2),    # 2 (counter)
          Card.new(:clubs, 5),     # Normal card
        ], status: :playing},
        %{id: "p3", name: "Player 3", hand: [
          Card.new(:clubs, 12),    # Queen (reverse)
          Card.new(:diamonds, 7),  # 7 (skip)
          Card.new(:spades, 11),   # Black Jack (attack)
          Card.new(:hearts, 2),    # 2 (counter)
          Card.new(:diamonds, 8),  # Normal card
        ], status: :playing},
        %{id: "p4", name: "Player 4", hand: [
          Card.new(:spades, 12),   # Queen (reverse)
          Card.new(:clubs, 7),     # 7 (skip counter)
          Card.new(:diamonds, 11), # Red Jack (cancel)
          Card.new(:clubs, 2),     # 2 (counter)
          Card.new(:spades, 9),    # Normal card
        ], status: :playing}
      ]

      game = %GameState{
        players: players,
        discard_pile: [Card.new(:hearts, 10)], # Start with hearts 10
    ...
    test "Queen + Skip + Attack combo maintains correct turn order", %{game: game} do
      ...
      assert game.current_player_index == 3  # Player 4 (counter-clockwise)
      ...
      assert game.current_player_index == 0  # Back to Player 1 (clockwise from 3)
      
      # Player 1 has no matching cards, must draw
      {:ok, game} = GameState.draw_cards(game, "p1", :cannot_play)
      assert game.current_player_index == 1  # Player 2
      
      # Player 2 plays Queen (matches rank)
      {:ok, game} = GameState.play_cards(game, "p2", [Card.new(:diamonds, 12)])
      assert game.direction == :counter_clockwise  # Reverses again
      assert game.current_player_index == 0  # Player 1 (counter-clockwise from 1)
    end
    ...
    test "Ace nomination during attack sequence", %{game: game} do
      ...
      assert game.pending_attack == {:twos, 6}  # Stacks to 6
    end
    ...
    test "Multiple direction reversals with complex effects", %{game: game} do
      ...
      {:ok, game} = GameState.play_cards(game, "p1", [Card.new(:hearts, 12)])
      assert game.direction == :counter_clockwise
      assert game.current_player_index == 3  # Player 4
    ...
    test "Deck exhaustion during complex attack sequence", %{game: game} do
      ...
      game = %{game | deck: [
        Card.new(:hearts, 3),
        Card.new(:spades, 4),
        Card.new(:clubs, 5)  # Only 3 cards left
      ]}
      
      # Player 1 creates massive Black Jack attack - update discard to match
      players = List.update_at(game.players, 0, fn p ->
        %{p | hand: [Card.new(:clubs, 11), Card.new(:spades, 11)]}  # Two Black Jacks
      end)
      game = %{game | players: players, discard_pile: [Card.new(:clubs, 10)]}  # Compatible top card
      
      {:ok, game} = GameState.play_cards(game, "p1", [
        Card.new(:clubs, 11),
        Card.new(:spades, 11)
      ])
      assert game.pending_attack == {:black_jacks, 10}  # 2 * 5 = 10
      ...
      assert length(player2.hand) >= 5  # Should have drawn some cards
      ...
      assert total_cards > 20  # Should have reasonable number of cards
    ...
    test "Winning during complex effect chain", %{game: game} do
      ...
      players = List.update_at(game.players, 0, fn p ->
        %{p | hand: [Card.new(:hearts, 12)]}  # Only a Queen
      end)
      game = %{game | players: players}
      ...
      {:ok, game} = GameState.play_cards(game, "p4", [Card.new(:spades, 12)])
      assert game.current_player_index == 1  # Player 2 (clockwise from 3, skipping won Player 1)
    end
  end
  ...
  describe "Game state integrity checks" do
    setup do
      ...
      players = [
        %{id: "p1", name: "Player 1", hand: [
          Card.new(:hearts, 12), Card.new(:clubs, 11), Card.new(:diamonds, 2)
        ], status: :playing},
        %{id: "p2", name: "Player 2", hand: [
          Card.new(:hearts, 11), Card.new(:spades, 7), Card.new(:clubs, 5)
        ], status: :playing}
      ]
    ...
    test "Card count remains constant throughout complex game", %{game: game} do
      ...
      {:ok, game} = GameState.draw_cards(game, "p2", :attack)  # P2 draws due to Black Jack attack
